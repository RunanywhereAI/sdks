    ...
    suspend fun performDifferentialUpdate(
        ...
        try {
            emit(DownloadProgress(
                ...
                progress = 0f,
                downloadedBytes = 0L,
                totalBytes = updateInfo.patchSizeBytes ?: modelInfo.sizeBytes,
                speedBytesPerSecond = null,
                isCompleted = false
            ))
            ...
            if (patchUrl != null) {
                ...
                downloadPatch(patchUrl, patchFile, modelInfo.id) { progress ->
                    emit(progress.copy(progress = progress.progress * 0.7f)) // 70% for download
                }
                ...
                applyPatch(existingFile, patchFile, updatedFile, modelInfo.id) { progress ->
                    emit(progress.copy(progress = 0.7f + progress.progress * 0.3f)) // 30% for applying
                }
                ...
                if (updatedHash == updateInfo.newSha256Hash) {
                    ...
                    emit(DownloadProgress(
                        modelId = modelInfo.id,
                        progress = 1f,
                        downloadedBytes = updateInfo.patchSizeBytes ?: modelInfo.sizeBytes,
                        totalBytes = updateInfo.patchSizeBytes ?: modelInfo.sizeBytes,
                        speedBytesPerSecond = null,
                        isCompleted = true
                    ))

...
class ConversationSummarizer(
    ...
    companion object {
        ...
        const val SUMMARY_TOKEN_LIMIT = 300
    ...
    suspend fun summarizeConversation(
        ...
        try {
            ...
            val summary = ConversationSummary(
                ...
                timeSpan = TimeSpan(
                    startTime = messagesToSummarize.first().timestamp.toEpochMilli(),
                    endTime = messagesToSummarize.last().timestamp.toEpochMilli()
                ),
        ...
        try {
            ...
            val updatedSummary = existingSummary.copy(
                ...
                timeSpan = existingSummary.timeSpan.copy(
                    endTime = newMessages.lastOrNull()?.timestamp?.toEpochMilli() ?: existingSummary.timeSpan.endTime
                ),
                summarizedAt = System.currentTimeMillis()
            )
    ...
    private fun filterMessagesForSummarization(
        ...
        return messages.filter { message ->
            ...
            options.timeFrameHours?.let { hours ->
                ...
                filtered.filter { it.timestamp.toEpochMilli() >= cutoffTime }
            } ?: filtered
        }
    }
    ...
    private fun extractParticipants(messages: List<Message>): List<String> {
        return messages
            .map { if (it.role.lowercase() == "user") "User" else "Assistant" }
            .distinct()
    }
...
data class SummarizationOptions(
    ...
    val maxLength: Int = ConversationSummarizer.SUMMARY_TOKEN_LIMIT,

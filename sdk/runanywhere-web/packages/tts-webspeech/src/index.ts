...
if (typeof window !== 'undefined') {
  try {
    ...
    registry.register('TTS' as AdapterType, 'webspeech', WebSpeechTTSAdapter as any);
    logger.info('Web Speech TTS adapter auto-registered', 'WebSpeechTTSAdapter');
  } catch (error) {
    // ServiceRegistry not available, skip auto-registration
    logger.debug('ServiceRegistry not available for auto-registration', 'WebSpeechTTSAdapter');
  }
}

// Named exports
export { WebSpeechTTSAdapter as default };
export const adapter = WebSpeechTTSAdapter;
